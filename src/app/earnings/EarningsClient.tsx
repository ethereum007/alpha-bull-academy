...
const VerdictBadge = ({
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium border ${styles[verdict]}`}
    >
      {icons[verdict]}
      {labels[verdict]}
    </span>
            ...
            <div className="space-y-1">
              ...
              <p
                className={`text-sm font-medium ${
                  result.revenueGrowth >= 0 ? "text-green-400" : "text-red-400"
                }`}
              >
                {result.revenueGrowth >= 0 ? "+" : ""}
                {result.revenueGrowth.toFixed(1)}% YoY
              </p>
            </div>
            <div className="space-y-1">
              <p className="text-xs text-muted-foreground uppercase tracking-wide">
              ...
              <p
                className={`text-sm font-medium ${
                  result.profitGrowth >= 0 ? "text-green-400" : "text-red-400"
                }`}
              >
                {result.profitGrowth >= 0 ? "+" : ""}
                {result.profitGrowth.toFixed(1)}% YoY
              </p>
            </div>
          </div>
          {result.analysis && (
...
const SectorSection = ({
  ...
  const avgGrowth =
    companies.reduce((acc, c) => acc + c.profitGrowth, 0) / companies.length;
          ...
          >
            <div className="inline-flex items-center gap-2 bg-primary/20 border border-primary/30 rounded-full px-4 py-2 mb-6">
              ...
              <span className="text-sm font-medium">
                {currentSeason.quarter} {currentSeason.fiscalYear} Earnings
                Season
              </span>
            </div>
            <h1 className="text-4xl md:text-5xl font-bold mb-4">
              Quarterly{" "}
              <span className="text-primary">Results Analysis</span>
            </h1>
            <p className="text-lg text-muted-foreground max-w-2xl mx-auto">
              Track earnings results, revenue growth, and profit trends for
              top Indian companies. Updated analysis for {currentSeason.period}.
            </p>
          </motion.div>
          ...
          <Card className="bg-gradient-to-r from-primary/20 to-accent/20 border-primary/30">
            <CardContent className="py-8 text-center">
              <h2 className="text-2xl font-bold mb-2">
                Want to Learn How to Analyze Results?
              </h2>
              <p className="text-muted-foreground mb-4">
                Join AlphaBull's trading courses to master fundamental and
                technical analysis.
              </p>
              <a
                href="/#courses"
